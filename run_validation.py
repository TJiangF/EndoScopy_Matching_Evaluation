...
def main(cfg):
    ...
    for i, data in enumerate(tqdm.tqdm(test_dataset)):
        ...
        list_desc_src = feature_extraction(patch_src, model, image_size)#This is the list of patches in frames(which vary), and the length of each patch is 128(n*1*128)
        
        list_desc_dst = feature_extraction(patch_dst, model, image_size)
        # if(i==1):
        #     print(len(list_desc_src[i][0]))
        #     # print(list_desc_dst[i][0][0])
        # print(len(list_desc_src[i]))
        # do matching
        # print(len(list_desc_src))
        # print(len(list_desc_dst))
