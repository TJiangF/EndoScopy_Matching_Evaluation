...
def matchesoutput(
    ...
    filenum = save_dst+'.txt'
    with open(filenum,'w') as f:
        for j in range(0, len(gt_keypoint_src)):
            xs = int(gt_keypoint_src[j][0])
            ys = int(gt_keypoint_src[j][1])
            xd = int(gt_keypoint_dst[matches[j]][0])
            yd = int(gt_keypoint_dst[matches[j]][1])
            
            f.write(str(xs)+' '+str(ys)+' '+str(xd)+' '+str(yd)+'\r\n')
    return 0
...
def feature_match(feature_vectors_1, feature_vectors_2, matching_threshold):
    matched_idx = []
    distance_list = []
    for i in range(0, len(feature_vectors_1)):#the number of features
        distance_sim = []
        ...
        candidate_index = np.argsort(distance_sim)[:1]
