    ...
    def load_data_path(self):
        ...
        for sequence in sequence_list:
            ...
            for match in curr_matches:
                ...
                src_abs_path = sequence+"/frame/"+src_filename
                # print(src_abs_path)
                # dst_abs_path = osp.join(sequence, "frames", dst_filename)
                dst_abs_path = sequence+"/frame/"+dst_filename
                # print(dst_abs_path)
...
class PatchDataset_demo(Dataset):
    """Sparse correspondences dataset."""
    ...
    def load_data_path(self):
        ...
        for sequence in sequence_list:
            # print(sequence)
            #  first get all gt matches for the current sequence
            # print(glob.glob(sequence+"/matches/*"))
            curr_matches = sorted(glob.glob(sequence+"/matches/*.txt"))
            for match in curr_matches:
                # get src and dst frame filenames

                # print(match)
                # print(osp.split(match)[1])
                # print(osp.splitext(osp.split(match)[1])[0].split("Frame")[1])
                frontlen = len(match)
                # print(frontlen)
                num = float(match[frontlen-7:-4])
                # _, num = osp.splitext(osp.split(match)[1])[0].split("Frame")
                # num = int(num)
                # print (int(num))
                src_filename = str(str(int(num)).zfill(4)) + ".png"
                # print(src_filename)
                num = str(int(num)+1)
                dst_filename = str(num.zfill(4)) + ".png"
                # print(dst_filename)
                # _, src_filename, dst_filename = osp.splitext(osp.split(match)[1])[0].split("_")
