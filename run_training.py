...
def main(cfg):
    ...
    output_dir = get_cwd()
    # output_dir = "/home/tf/Documents/Image_Understanding_project/Self-Supervised-Endoscopic-Image-Key-Points-Matching/train_result"
    ...
    model = HardNet128().to(device)#revised
    ...
    print(f"num of epochs: {nb_epoch}")
    for epoch in range(nb_epoch):
        ...
        for (idx, data) in enumerate(train_loader):
            _, inputs = data
            input_var = torch.autograd.Variable(inputs).to(device)#revised
            if not (list(input_var.size())[0] == batch_size):
                continue

            inputs_var_batch = input_var.view(batch_size * 3, 1, image_size, image_size).to(device)#revised

            # computed output
            output1 = model(inputs_var_batch).to(device)#revised
            output = output1.view(output1.size(0), -1).cuda()#revised cpu()
